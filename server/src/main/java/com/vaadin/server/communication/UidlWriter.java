...
public class UidlWriter implements Serializable {
	 ...
	 *
	 * @param ui     The {@link UI} whose changes to write
	 * @param writer The writer to use
	 * @param async  True if this message is sent by the server asynchronously,
	...
	public void write(UI ui, Writer writer, boolean async) throws IOException {
		...
		while (true) {
			...
			for (ClientConnector connector : connectorsToProcess) {
				...
				boolean initialized = uiConnectorTracker.isDirty(connector)
						&& uiConnectorTracker.isClientSideInitialized(connector);
		...
		try {
			...
			/*
			 * Ensure super classes come before sub classes to get script dependency order
			 * right. Sub class @JavaScript might assume that
			 *
			...
			assert (uiConnectorTracker.getDirtyConnectors().isEmpty())
					: "Connectors have been marked as dirty during the end of the paint phase. This is most certainly not intended.";
	...
	private void writePerformanceData(UI ui, Writer writer) throws IOException {
		if (!ui.getSession().getService().getDeploymentConfiguration().isProductionMode()) {
			writer.write(String.format(", \"timings\":[%d, %d]", ui.getSession().getCumulativeRequestDuration(),
